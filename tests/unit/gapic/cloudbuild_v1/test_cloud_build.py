...
def test_cloud_build_client_client_options_from_dict():
    with mock.patch(
        "google.devtools.cloudbuild_v1.services.cloud_build.transports.CloudBuildGrpcTransport.__init__"
    ) as grpc_transport:
...
def test_list_builds_pager():
    ...
    with mock.patch.object(type(client._transport.list_builds), "__call__") as call:
        ...
        results = [i for i in client.list_builds(request={},)]
...
def test_list_build_triggers_pager():
    ...
    with mock.patch.object(
        ...
        results = [i for i in client.list_build_triggers(request={},)]
        assert len(results) == 6
