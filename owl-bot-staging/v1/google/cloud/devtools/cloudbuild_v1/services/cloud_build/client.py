...
class CloudBuildClient(metaclass=CloudBuildClientMeta):
    ...
    @staticmethod
    def github_enterprise_config_path(project: str,config: str,) -> str:
        """Returns a fully-qualified github_enterprise_config string."""
        return "projects/{project}/githubEnterpriseConfigs/{config}".format(project=project, config=config, )
    ...
    def parse_github_enterprise_config_path(path: str) -> Dict[str,str]:
        ...
        m = re.match(r"^projects/(?P<project>.+?)/githubEnterpriseConfigs/(?P<config>.+?)$", path)
        return m.groupdict() if m else {}
