...
async def test_list_builds_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.list_builds(request={})).pages: # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ['abc','def','ghi', '']):
            assert page_.raw_page.next_page_token == token
...
async def test_list_build_triggers_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.list_build_triggers(request={})).pages: # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ['abc','def','ghi', '']):
            assert page_.raw_page.next_page_token == token
...
async def test_list_worker_pools_async_pages():
    ...
    with mock.patch.object(
        ...
        pages = []
        async for page_ in (await client.list_worker_pools(request={})).pages: # pragma: no branch
            pages.append(page_)
        for page_, token in zip(pages, ['abc','def','ghi', '']):
            assert page_.raw_page.next_page_token == token
...
def test_create_worker_pool_rest(request_type):
    ...
    request_init["worker_pool"] = {'name': 'name_value', 'display_name': 'display_name_value', 'uid': 'uid_value', 'annotations': {}, 'create_time': {'seconds': 751, 'nanos': 543}, 'update_time': {}, 'delete_time': {}, 'state': 1, 'private_pool_v1_config': {'worker_config': {'machine_type': 'machine_type_value', 'disk_size_gb': 1261}, 'network_config': {'peered_network': 'peered_network_value', 'egress_option': 1}}, 'etag': 'etag_value'}
    request = request_type(**request_init)
