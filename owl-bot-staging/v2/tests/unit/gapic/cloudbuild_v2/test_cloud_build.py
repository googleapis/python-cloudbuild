...
def test_cloud_build_client_client_options_from_dict():
    with mock.patch('google.devtools.cloudbuild_v2.services.cloud_build.transports.CloudBuildGrpcTransport.__init__') as grpc_transport:
...
def test_cloud_build_base_transport():
    ...
    with mock.patch('google.devtools.cloudbuild_v2.services.cloud_build.transports.CloudBuildTransport.__init__') as Transport:
...
def test_cloud_build_base_transport_with_credentials_file():
    ...
    with mock.patch.object(google.auth, 'load_credentials_from_file', autospec=True) as load_creds, mock.patch('google.devtools.cloudbuild_v2.services.cloud_build.transports.CloudBuildTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
...
def test_cloud_build_base_transport_with_adc():
    ...
    with mock.patch.object(google.auth, 'default', autospec=True) as adc, mock.patch('google.devtools.cloudbuild_v2.services.cloud_build.transports.CloudBuildTransport._prep_wrapped_messages') as Transport:
        Transport.return_value = None
        adc.return_value = (ga_credentials.AnonymousCredentials(), None)
        transport = transports.CloudBuildTransport()
        adc.assert_called_once()
