...
from google.cloud.devtools.cloudbuild_v2.services.repository_manager import RepositoryManagerAsyncClient
...
def test_fetch_git_refs(request_type, transport: str = 'grpc'):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
            type(client.transport.fetch_git_refs),
            '__call__') as call:
        # Designate an appropriate return value for the call.
...
def test_fetch_git_refs_rest(request_type):
    ...
    with mock.patch.object(type(client.transport._session), 'request') as req:
        ...
        response_value = Response()
        response_value.status_code = 200
        pb_return_value = repositories.FetchGitRefsResponse.pb(return_value)
        json_return_value = json_format.MessageToJson(pb_return_value)
...
def test_fetch_git_refs_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
         mock.patch.object(path_template, "transcode")  as transcode, \
         mock.patch.object(transports.RepositoryManagerRestInterceptor, "post_fetch_git_refs") as post, \
         mock.patch.object(transports.RepositoryManagerRestInterceptor, "pre_fetch_git_refs") as pre:
        pre.assert_not_called()
        post.assert_not_called()
